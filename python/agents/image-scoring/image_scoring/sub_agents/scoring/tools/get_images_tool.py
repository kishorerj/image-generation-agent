...
async def get_image(tool_context: ToolContext):
    try:
        ...
        prompt = """
        "Score the following image {artifact} based on this policy: {policy}."\
         "Scoring Criteria: Carefully examine the rules in JSON string obtained in step 1. For EACH rule described within this JSON string:"
        ...
        response = client.models.generate_content(model=model, contents=[prompt, artifact])
        ...
        return {
            "status": "error",
            "message": f"Error processing artifact {artifact_name}: {str(e)}",
        }
